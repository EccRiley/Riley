...
#' ---
#'
#+ knitOpts1, echo=FALSE
knitr::knit_hooks$set(
    RplotA = function(before, options, envir) {
        if (before) {
            par(bg = "transparent",
            family = "ETBembo",
            cex = 0.95)
...
#'
#+ echo=TRUE, tidy=TRUE
# Sys.setenv(PATH=paste(Sys.getenv("PATH"),"/opt/X11/bin", "/Library/TeX/texbin",sep=":")) ## Rapp (i.e., the R console) keeps forgetting my latex path... so I'm just adding this because it can't really hurt even if R.app suddenly remembers the path variable. ##
#'
#' Miscellaenous options set to my personal preference for how my `R console` looks.
#'
options(prompt = "> ",
        continue = "... ",
        width = 57)
