    ...
    if (th == "A") {
        ...
        thm <- function(base_size = 12) {
            ...
            thm_A(base_size) %+replace%
                theme(axis.text.x = element_text(colour = pal_rye[20], size = rel(.75)))#,)
        }
    } else if (th == "C") {
        thm <- function(base_size = 12) {
            ## thm_C() ##
            thm_A(base_size) %+replace%
                theme(
                    axis.text.x = element_text(colour = pal_rye[20], size = rel(.65)),
                    legend.title = element_blank(),
                    legend.text = element_text(colour = pal_rye[20], size = rel(0.65)),
